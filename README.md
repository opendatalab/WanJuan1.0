...
书生·万卷多模态预训练语料库是上海人工智能实验室（上海AI实验室）联合中央广播电视总台、人民网、国家气象中心、中国科学技术信息研究所等大模型数据联盟成员，联合打造的多知识、多模态、标准化的高质量语料数据，该语料库已覆盖通用及专用领域，总量超过10TB。

通用领域集纳跨模态、多领域、多语种知识语料，涵盖文本、语音、图像、视频、三维等多种数据模态；覆盖中文、英文、阿拉伯语和日语等200+种多语言；并支持语义理解、知识对话、内容创作、逻辑推理、代码生成等各类生成式任务；在数据的采集、加工和汇总建设上满足对数据的多样性、无害性和有益性的要求。

专用领域语料库针对特定行业而设，覆盖科学、医疗、教育、法律、交通等领域，并基于各领域的数据特征和形式，构建了专业分类体系。科学方面，包含物质科学、气象领域和生物科学等大规模科研数据；医疗方面不仅覆盖的广泛医疗知识，还包括不同模态的医疗影像及多种疾病的诊断数据；教育方面包含从初、中、高等教育及职业教育各阶段的教材教辅和试题等数据。
...
- 在精细处理方面，书生·万卷1.0经历了语言甄别、正文抽取、格式标准化、基于规则及模型的数据过滤与清洗、多尺度去重、数据质量评估等精细化数据处理环节，因而能更好地适配后续的模型预训练需求。
- 在价值对齐方面，研究人员在书生·万卷1.0的构建过程中，着眼于内容与中文主流价值观的对齐，通过算法与人工评估结合的方式，提升了语料的纯净度。
- 在易用高效方面，研究人员在书生·万卷1.0采用统一格式，并提供详细的字段说明和工具指导，使其兼顾了易用性和效率，可快速应用于语言、多模态等大模型预训练。

书生·万卷1.0为书生·万卷多模态预训练语料库的首个开源版本，包含了书生·万卷文本数据集、书生·万卷图文数据集、书生·万卷视频数据集三部分，数据总量超过1.9TB。

目前，书生·万卷1.0已被应用于书生·多模态、书生·浦语的预训练。通过对高质量语料的“消化”，书生系列模型在语义理解、知识问答、视觉理解、视觉问答等各类生成式任务表现出的优异性能。
...
书生·万卷视频数据集1.0由央视贡献，包含多种类型的节目影像，总计视频文件数1074 个，数据大小超过800GB。内容覆盖军事、文艺、体育、自然、真实世界、知识类、影像艺术、媒体、美食、历史纪录片、科教类等方面。
...
（OpenDataLab）倡导和谐友好的开源交流环境，若在开源数据集内发现有侵犯您合法权益的内容，可发送邮件至（OpenDataLab@pjlab.org.cn），邮件中请写明侵权相关事实的详细描述并向我们提供相关的权属证明资料。我们将于3个工作日内启动调查处理机制，并采取必要的措施进行处置。但您应确保您投诉的真实性，否则采取措施后所产生的不利后果应由您独立承担。
